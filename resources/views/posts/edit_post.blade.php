...
<div class="row d-flex justify-content-center align-items-center h-100 mt">
  <div class="col-xl-6">
    <div class="card-custom">
      ...
      <div class="card-body">
        <form action="{{ route('confirmEdit', $posts->id) }}" method="get">
          @csrf
          <!-- Email input -->
          <div class="mb-3 mt-3 row d-flex">
            <label for="email" class="control-label col-sm-3">Title:</label>
            <div class="col-sm-9">
              <!--<input type="text" class="form-control" id="title" name="title" value=@if(old('title')) "{{old('title') }}" @else "{{ $posts->title }}" @endif>-->
              <input type="text" class="form-control" id="title" name="title" value="@if(old('title')) {{ old('title') }} @else {{ $posts->title }} @endif">
              @error('title')
          ...
          <div class="mb-3 row d-flex">
            ...
            <div class="col-sm-9">
              <textarea class="form-control" rows="5" id="description" name="description">{{ $posts->description }}</textarea>
              @error('description')
                <span class="text-danger">{{$message}}</span>
              @enderror
            </div>
          ...
          <div class="row d-flex justify-content-center align-content-center">
            <div class="col-sm-6">
              <button type="submit" data-mdb-button-init data-mdb-ripple-init class="btn btnColor btn-block col-sm-4">Edit</button>
              <button type="button" onclick="resetTitle()" data-mdb-button-init data-mdb-ripple-init class="btn btn-secondary btn-block col-sm-4">Clear</button>
            </div>
          </div>
        </form>
      </div>
